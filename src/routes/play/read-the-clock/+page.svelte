<script lang="ts">
	...
	let question = $state<ReadClockQuestion>(
		generateReadClockQuestion(currentLevel(), 'analogToDigital')
	);

	function makeQuestion(): ReadClockQuestion {
		return generateReadClockQuestion(currentLevel(), 'analogToDigital');
	}
...
<h1 class="visually-hidden">{$t('nav.readClock')}</h1>

<div class="target">
	<button class="speak-wrap" onclick={speakTarget} aria-label={$t('explore.speakButton')}>
		<AnalogClockFace time={question.targetTime} size="min(50vw, 32vh, 190px)" />
	</button>
</div>
	...
	<div class="choice-grid">
		{#each question.choices as choice, i (i)}
			<ChoiceButton onclick={() => choose(choice)}>
				<DigitalClockDisplay time={choice} />
			</ChoiceButton>
		{/each}
	</div>
{/if}
